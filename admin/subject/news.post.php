<?php 
	...
	if($news_id==0){
		$sub_item = new table_class('fb_subject_items');
		$sub_item->subject_id = $_POST['subject_id'];
		$sub_item->category_id = $_POST['subject_cid'];
		$sub_item->category_type = 'news';
		$sub_item->resource_id = $news->id;
		$sub_item->save();
	}else{
		$sub_item = new table_class('fb_subject_items');
		$sub_item->category_id = $_POST['subject_cid'];
