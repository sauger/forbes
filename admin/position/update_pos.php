...
function update_pos($category_name,$count=1,$pos_name,$has_children=true,$ignore_time = false){
	...
	$sql = "select distinct(title) as title,id,short_title,created_at,description,video_photo_src,author from fb_news where is_adopt=1 and (block_endtime = '0000-00-00 00:00:00' or block_endtime is null or block_endtime <= now()) and category_id in ($ids)  order by created_at desc limit {$count}";
	#echo $sql;
	$news = $db->query($sql);
...
function update_click($limit,$position_name){
	...
	$sql = "select title,id,short_title,created_at,description,video_photo_src from fb_news where 1=1 and is_adopt=1 and created_at > '{$date}' group by title order by click_count desc,created_at desc limit {$limit}";
	$news = $db->query($sql);
	$news_count = $db->record_count;
	for($i=0;$i<$news_count;$i++){
		for($j=0;$j<$limit;$j++){
