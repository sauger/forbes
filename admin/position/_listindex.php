...
function update_list_pos($pos){
	...
	$type = pos_type($pos);
	$items = $db->query("select name,comment,image_src,id from fb_custom_list_type where position = {$pos} and use_pos=1 and is_adopt=1 order by created_at desc limit 4");
	...
	for($i=0;$i<$len; $i++){
		...
		$exist_items[$i]->static_href= '/list/'.$items[$i]->id;
